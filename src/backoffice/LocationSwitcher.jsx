...
export default function LocationSwitcher({ onClose }) {
  const [items, setItems] = useState([]); // { company, locations[] }
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data: profile } = await supabase.from('user_profiles').select('role, location_id').eq('id', user.id).single();
      const superAdmin = profile?.role === 'super_admin';
      setIsSuperAdmin(superAdmin);

      if (superAdmin && platformSupabase) {
        // Super admin: load ALL companies + ALL their locations from Platform DB
        const { data: companies } = await platformSupabase.from('companies').select('id, name, slug, plan').order('name');
        if (companies?.length) {
          const { data: locations } = await platformSupabase.from('locations').select('id, company_id, name, ops_location_id, ops_db_url').order('name');
        ...
        // Regular user: just show their own location(s) from user_profiles
        const locId = profile?.location_id;
        if (locId) {
          const { data: loc } = await supabase.from('locations').select('id, name, address').eq('id', locId).single().catch(() => ({ data: null }));
          setItems([{ company: { name: 'Your location' }, locations: loc ? [{ id: locId, name: loc.name || locId, ops_location_id: locId }] : [{ id: locId, name: locId, ops_location_id: locId }] }]);
        }
      }
  ...
  const switchTo = async (loc) => {
    setSwitching(loc.id);
    // The ops_location_id is what the app uses to query the Ops DB
    ...
    // Update user_profiles so it persists on next login
    try {
      const { data: { user } } = await supabase.auth.getUser();
  ...
  // Flatten all locations for current check
      ...
      <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', zIndex:9999, width:480, maxWidth:'92vw', maxHeight:'80vh', display:'flex', flexDirection:'column', background:'var(--bg1)', border:'1px solid var(--bdr)', borderRadius:16, boxShadow:'0 24px 80px rgba(0,0,0,0.5)', overflow:'hidden' }}>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'12px 16px 20px' }}>
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding:'32px 0', color:'var(--t4)', fontSize:13 }}>No locations found</div>
          ) : (
            items.map(({ company, locations }) => (
              <div key={company.id || company.name} style={{ marginBottom:16 }}>
                {/* Company header */}
                ...
                {locations.length === 0 ? (
                  ...
                  return (
                    <button key={loc.id} onClick={() => !isCurrent && switchTo(loc)}
                      ...
                      style={{ width:'100%', textAlign:'left', padding:'13px 16px', marginBottom:6, borderRadius:12, border:`1.5px solid ${isCurrent ? 'var(--acc-b)' : 'var(--bdr)'}`, background:isCurrent ? 'var(--acc-d)' : 'var(--bg3)', cursor:isCurrent ? 'default' : 'pointer', fontFamily:'inherit', transition:'all .12s', display:'flex', alignItems:'center', gap:12, opacity:switching && !isLoading ? 0.5 : 1 }}
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        {isSuperAdmin && loc.ops_location_id && (
                          <div style={{ fontSize:10, color:'var(--t4)', fontFamily:'var(--font-mono)' }}>{loc.ops_location_id.slice(0, 16)}…</div>
                        )}
                      </div>
        ...
        {isSuperAdmin && (
          <div style={{ padding:'10px 24px 14px', borderTop:'1px solid var(--bdr)', fontSize:11, color:'var(--t4)', textAlign:'center' }}>
            Super admin view — showing all organisations and locations
          </div>
        )}
      </div>
    </>
