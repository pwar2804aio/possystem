...
function ShiftBar({ shift }) {
  return (
    <div style={{
      ...
      background: 'var(--bg1)', borderBottom: '1px solid var(--bdr2)',
      flexShrink: 0,
    }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '0 20px', flex: 1 }}>
        <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20, background: 'var(--grn-d)', border: '1px solid var(--grn-b)', color: 'var(--grn)' }}>
          ● {shift.name}
        </span>
...
function Sidebar({ surface, setSurface }) {
  ...
  return (
    <nav style={{
      width: 'var(--nav)', background: 'var(--bg1)',
  ...
  return (
    <div style={{ position: 'relative', marginBottom: 8 }}>
      <div onClick={() => setOpen(o => !o)} style={{
        ...
        background: staff.color + '22', border: `2px solid ${staff.color}55`,
      ...
      {open && (
        <div style={{
          ...
          <button onClick={() => { logout(); setOpen(false); }} style={{
            ...
            fontSize: 13, textAlign: 'left', fontFamily: 'inherit', fontWeight: 500,
          }}>Sign out</button>
        </div>
      )}
    </div>
...
function Toast({ toast }) {
  ...
  return (
    <div className="toast" key={toast.key} style={{ background: c.bg, border: `1px solid ${c.bdr}`, color: c.color }}>
      {toast.msg}
    </div>
  );
}
