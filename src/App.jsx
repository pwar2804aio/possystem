...
const CHANGELOG = [
  ...
  },
  {
    ...
    changes: [
      ...
      'Removed the redundant inline allergen banner from inside the items area — top bar carries it now, no need to repeat. Items area gets that vertical space back for the menu grid.',
      'TASK 2 — Product modal (KioskProductModal) full redesign to match the reference Peter shared. Hero image at top with a circular X close button in the top-right (was ← in a glass-morph square top-left). Title in brand color (was neutral text). Base price now displayed as a large brand-color number under the description (previously only shown via the bottom CTA). Allergens row reformatted: inline test-tube/drop SVG icon + "Allergens" label + comma-separated list, all in brand color (was a row of pill chips).',
      'Modifier groups: group name now in BRAND COLOR (was neutral) and bigger (22→30px max), with a "Select options" / hint subtitle in brand color underneath. Option cards now have brand-color names (was neutral) and bigger touch targets.',
      'Modifier OPTION images and descriptions are now supported in the kiosk modal. The option data shape already carried opt.image + opt.description (other surfaces like POS InlineItemFlow use them), but the kiosk modal was ignoring those fields. Now: opt.image renders as a full-width 16:9 image at the top of the option card; opt.description renders as a smaller muted line under the option name. Existing options without images/descriptions render unchanged.',
      'Layout adapts to content: when ANY option in a group has an image OR description, the option grid switches from 2 columns to 1 column so rich content gets full horizontal space. Simple text-only modifier groups stay as a tidy 2-col grid matching the reference. This means the kiosk auto-scales: assign rich modifier content in BO, customers see a layout that does it justice; assign simple boolean toggles, customers see a compact compare-at-a-glance grid.',
      'Sub-group options also support image + description, with the image rendered as a small 48px thumbnail to the left of the text. Same gracefully-renders-when-absent behavior.',
      'Quantity stepper buttons in the bottom CTA bar are now brand-color filled (were neutral surface-colored). The Add CTA pill no longer fully rounded — uses radius:18 to match the new screen aesthetic. Both stepper and CTA use clamp() sizing for consistent scale across kiosk screen sizes.',
      'Modal subscribes to language changes via useKioskLang() so if the customer changes language behind the modal (unlikely in practice) the strings update. Added i18n keys: product.allergens, .selectOptions, .required, .optional, .pickOne, .pick, .upTo, .anythingElse, .anythingElse.placeholder, .addToOrder, .loading, plus 3 new menu.allergens.* keys for the prominent banner. All translated to en/es/fr/de/it/pt.',
      'Behavior fully preserved: same validation rules (min/max per group, nested sub-group validation, instruction-group enforcement), same selection state shape, same nested-modifier inline expansion, same line-key generation for cart, same Add flow. Pure UI rework + i18n + new opt.image/opt.description rendering.',
    ],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      'Wave 7: Location switch flow unchanged — the existing setResolvedLocationId + rpos-bo-location localStorage + user_profiles.location_id update + window.location.reload sequence still handles the cutover. Reports re-fetch on mount so the new location takes effect immediately after the reload.',
      'Cleanup: Removed ~160 lines of dead code from src/backoffice/sections/BOReports.jsx — LegacyPMix, LegacyServers, LegacyTax function bodies plus the legacyStats useMemo block that only fed them. Replaced with a single lightweight totalRevenue useMemo (used in the checks count subtitle). Removed the now-unused calculateOrderTax import. LegacyOpen stays until the Open orders report is rebuilt later.',
      'Sprint wrap — reports module: 18 reports across 7 categories (Sales, Exceptions, Staff, Fiscal, Orders, Kitchen, Location) shipped across 8 wave commits (v4.6.15 through v4.6.23) plus 2 hotfixes + 2 Supabase migrations run in production. Every report: period filter with compare chips, CSV export where it adds value, empty states that read well. Tip pooling calculator as the US-market differentiator. Z-report prints to Sunmi thermal or saves as PDF from the same button. Location compare detects outliers via portfolio medians. Both schema migrations (tax_amount + staff_id on closed_checks, user_locations junction) are live in POSUP prod.',
      'Deferred to Wave 8+: user_locations management UI for owners (invite / remove users per site — currently requires direct Supabase table edits), role-based UI restrictions (staff vs manager permissions), write RLS scoped per active location, PDF-native export for non-Z-report reports (currently uses browser print dialog), true table turn time (needs seated_at column on closed_checks), real staff clock-in/out (needs shift_sessions table), menu item cost for true Menu engineering contribution margin (needs menu_items.cost column), tip pool rule persistence (needs tip_pool_rules table), z_reports audit table for auto-incrementing Z numbers.',
    ],
  },
  {
    version: '4.6.22', date: '22 Apr 2026', label: 'Reports wave 6 — multi-location compare + Location category filled',
    changes: [
      'DEPLOY STEP REQUIRED: Run supabase/migrations/20260422_multi_location.sql in the Supabase SQL editor BEFORE this version goes to production. Creates the user_locations junction table, seeds existing user_profiles.location_id rows into it as manager role, and adds additive RLS policies on closed_checks + kds_tickets so users can read data for any location listed in their user_locations rows. Additive + idempotent, safe to re-run.',
      'DB: New user_locations(user_id, location_id, role, created_at) junction table. Role is one of owner | manager | staff | viewer. Indexed on both sides. RLS restricts each user to their own rows. Seeded from existing user_profiles so existing single-location users get one row automatically — nothing breaks on day one.',
      'DB: Two new helpers in src/lib/db.js. fetchAccessibleLocations tries the user_locations junction first and falls back to user_profiles.location_id for pre-migration environments, returning the array of locations the user can read. fetchClosedChecksMultiRange runs the per-location fetches in parallel and tags each row with its source locationId so downstream aggregations can group by site.',
      'New: Location compare report, populating the previously-empty Location category card. Fetches closed_checks across every site the authenticated user has access to (via user_locations), in parallel, for the selected date range. Four portfolio tiles at the top (combined revenue with median site revenue as subtitle, total covers, portfolio avg check, total tips). Then an outlier alerts strip that flags any location whose void rate, refund rate, discount rate or tip rate is 2x+ the portfolio MEDIAN and above an absolute floor (keeps low-volume sites from triggering spurious alerts). Then a sortable per-location table with rank, name, role, checks, covers, revenue, avg check, tip %, void %, disc %, and a vs-median delta column colored green/red. CSV export included.',
      'Alert thresholds: void > 2x median and > 2.0% absolute, refunds > 2x median and > 1.5%, discounts > 2x median and > 5.0%, tip rate BELOW half the median (and median >= 5%). Medians are robust to outliers — one bad site does not drag the reference point.',
      'UX: If the user has only one location, the Location compare report shows a friendly empty state explaining that compare kicks in once more sites are linked. If the migration has not run, the helper falls back to user_profiles and the report still renders with the single-location empty state.',
      'Not yet shipped (Wave 7): a location picker in the BO topbar for switching the active location used by single-location reports and by the POS. All other reports continue to be scoped to the single currently-configured location. Also coming in Wave 7: write RLS scoped to active location with role-based permissions, and a user_locations management UI for owners.',
    ],
  },
  {
