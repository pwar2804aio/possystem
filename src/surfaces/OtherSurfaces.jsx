...
function BOPrinters({ showToast }) {
  const { printJobs } = useStore();
  const [printers, setPrinters] = useState(PRINTERS);
  const testPrint = (p) => {
    showToast(`Test print sent to ${p.name}`, 'info');
  };
  ...
  return (
    <>
      <div style={{ fontSize:17, fontWeight:600, marginBottom:20 }}>Printer setup & routing</div>

      {/* Recent print jobs */}
      {recentJobs.length > 0 && (
        <div style={{ marginBottom:24 }}>
          <div style={{ fontSize:13, fontWeight:700, color:'var(--t2)', marginBottom:10 }}>Recent print jobs</div>
          <div style={{ background:'var(--bg3)', border:'1px solid var(--bdr)', borderRadius:12, overflow:'hidden' }}>
            {recentJobs.map((job, i) => (
              <div key={job.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 14px', borderBottom:i<recentJobs.length-1?'1px solid var(--bdr)':'none', fontSize:12 }}>
                <div style={{ width:8, height:8, borderRadius:'50%', background:'var(--grn)', flexShrink:0 }}/>
                <span style={{ fontWeight:600, color:'var(--t1)', minWidth:60 }}>{job.tableLabel}</span>
      ...
      {/* Production centres */}
      <div style={{ marginBottom:24 }}>
        <div style={{ fontSize:13, fontWeight:700, color:'var(--t2)', marginBottom:10 }}>Production centre routing</div>
            ...
            <div key={pc.id} style={{ background:'var(--bg3)', border:'1px solid var(--bdr)', borderRadius:12, padding:'14px 16px', marginBottom:8, display:'flex', alignItems:'center', gap:12 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:'var(--t3)', marginTop:2 }}>
                  → {printer?.name || 'No printer assigned'} · {pc.type}
                </div>
              </div>
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                {printer && <div style={{ width:8, height:8, borderRadius:'50%', background:printer.status==='online'?'var(--grn)':'var(--red)' }}/>}
                <button className="btn btn-ghost btn-sm" onClick={()=>showToast(`Reassign ${pc.name} printer — coming in V2`,'info')}>Reassign</button>
              </div>
            </div>
          );
        })}
      ...
      {/* Printers on network */}
      <div>
        <div style={{ fontSize:13, fontWeight:700, color:'var(--t2)', marginBottom:10 }}>Printers on network</div>
        {printers.map(p=>(
        ...
        <button className="btn btn-ghost" style={{marginTop:8}} onClick={()=>showToast('Scanning network for Sunmi NT311 printers…','info')}>
          + Scan for printers
        </button>
      </div>
    </>
