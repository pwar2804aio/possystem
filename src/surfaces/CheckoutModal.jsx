        ...
        <div style={{ display:'flex', gap:0, borderBottom:'1px solid var(--bdr)', overflowX:'auto' }}>
          {['review','tip','method'].map((s,i) => (
            <div key={s} style={{
              ...
              borderBottom: step===s?'2px solid var(--c-acc)':'2px solid transparent',
        ...
        <div style={{ padding:'20px 24px' }}>
          ...
          {step==='review' && (
            <>
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:12, fontWeight:600, color:'var(--c-text3)', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:10 }}>Order items</div>
                {items.map(item => (
                  <div key={item.uid} style={{ borderBottom:'1px solid var(--bdr)', paddingBottom:10, marginBottom:10 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', gap:8 }}>
                      <div style={{ flex:1 }}>
                        ...
                        {item.mods?.map((m,i) => (
                          <div key={i} style={{ fontSize:12, color:'var(--c-text3)', marginTop:2, display:'flex', justifyContent:'space-between' }}>
                        ...
                        {item.allergens?.length>0 && (
                          <div style={{ fontSize:11, color:'var(--c-red)', marginTop:2 }}>
                            ⚠ {item.allergens.map(a=>ALLERGENS.find(x=>x.id===a)?.label).filter(Boolean).join(' · ')}
                          </div>
                        )}
                      </div>
                      <div style={{ fontSize:14, fontWeight:700, color:'var(--c-acc)', whiteSpace:'nowrap' }}>
                        £{(item.price * item.qty).toFixed(2)}
                      </div>
                    </div>
                    {item.qty>1 && <div style={{ fontSize:11, color:'var(--c-text3)', marginTop:2 }}>£{item.price.toFixed(2)} each</div>}
                  </div>
                ))}
              </div>

              <div style={{ background:'var(--c-raised)', borderRadius:12, padding:'12px 16px', marginBottom:20 }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'var(--c-text3)', marginBottom:4 }}>
                ...
                {service===0 && (
                  <div style={{ fontSize:12, color:'var(--c-grn)', marginBottom:4 }}>No service charge · {orderType}</div>
                )}
          ...
          {step==='tip' && (
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:6, marginBottom:16 }}>
                {[0,10,12.5,15,20].map(p => (
                  <button key={p} onClick={() => { setTipPct(p); setCustomTip(''); }} style={{
                    ...
                    <div style={{ fontSize:13, fontWeight:700, color:tipPct===p&&customTip===''?'var(--c-acc)':'var(--c-text)' }}>{p}%</div>
              ...
              <div style={{ background:'var(--c-raised)', borderRadius:12, padding:'12px 16px', marginBottom:18 }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'var(--c-text3)', marginBottom:4 }}><span>Bill</span><span>£{total.toFixed(2)}</span></div>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'var(--c-text3)', marginBottom:4 }}><span>Tip ({tipPct!==null?`${tipPct}%`:'custom'})</span><span>£{tipAmt.toFixed(2)}</span></div>
                <div style={{ height:1, background:'var(--bdr)', margin:'8px 0' }}/>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:18, fontWeight:700 }}><span>Grand total</span><span style={{color:'var(--c-acc)'}}>£{grand.toFixed(2)}</span></div>
              </div>
          ...
          {step==='method' && (
            <>
              ...
              <div style={{ fontSize:13, color:'var(--c-text3)', marginBottom:20 }}>Tip included: £{tipAmt.toFixed(2)}</div>
              {[
                ...
                <div key={m.id} style={{ padding:16, background:'var(--c-raised)', borderRadius:12, cursor:'pointer', border:'1px solid var(--bdr)', display:'flex', alignItems:'center', gap:14, marginBottom:8, transition:'all .12s' }}
                  onMouseEnter={e=>e.currentTarget.style.borderColor='var(--c-acc-bdr)'}
                  onMouseLeave={e=>e.currentTarget.style.borderColor='var(--bdr)'}
                  onClick={() => setStep(m.id)}>
                  <div style={{ fontSize:28 }}>{m.icon}</div>
                  <div><div style={{ fontWeight:500, fontSize:14 }}>{m.label}</div><div style={{ fontSize:12, color:'var(--c-text3)', marginTop:2 }}>{m.sub}</div></div>
                </div>
              ))}
            </>
          )}
          ...
          {step==='card' && (
            <div style={{ textAlign:'center', padding:'32px 0' }}>
              ...
              <div style={{ fontSize:14, color:'var(--c-text3)', marginBottom:32 }}>Present card to Stripe Reader S700</div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'10px 20px', background:'var(--c-acc-dim)', border:'1px solid var(--c-acc-bdr)', borderRadius:20, fontSize:13, color:'var(--c-acc)', marginBottom:32 }}>
          ...
          {step==='cash' && (
            <>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
                <span style={{ fontSize:24, color:'var(--c-text3)' }}>£</span>
              ...
              {cash && parseFloat(cash) >= grand && (
                <div style={{ background:'var(--c-grn-dim)', border:'1px solid var(--c-grn-bdr)', borderRadius:14, padding:'16px 20px', marginBottom:20, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <span style={{ fontSize:15, color:'var(--c-grn)', fontWeight:500 }}>Change due</span>
                  <span style={{ fontSize:28, fontWeight:700, color:'var(--c-grn)' }}>£{change.toFixed(2)}</span>
                </div>
              )}
          ...
          {step==='split' && (
            <>
              ...
              <div style={{ background:'var(--c-raised)', borderRadius:12, padding:'14px 18px', marginBottom:20 }}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}><span style={{ color:'var(--c-text3)' }}>Total</span><span>£{total.toFixed(2)}</span></div>
                <div style={{ display:'flex', justifyContent:'space-between' }}>
                  <span style={{ fontSize:15, fontWeight:500 }}>Each person pays</span>
                  <span style={{ fontSize:26, fontWeight:700, color:'var(--c-acc)' }}>£{(total/splits).toFixed(2)}</span>
                </div>
              </div>
              {Array.from({length:splits}).map((_,i)=>(
                <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 12px', background:'var(--c-raised)', borderRadius:8, marginBottom:6 }}>
                  <span style={{ fontSize:13, color:'var(--c-text2)' }}>Guest {i+1}</span>
